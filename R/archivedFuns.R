...
prune <- function(oldTreee,
  ...
  savedGrove <- lapply(seq_len(numberOfPruning), function(i) new_SingleTreee(datX = datX[idxCV!=i, , drop = FALSE],
  ...
  treeeForPruning <- lapply(savedGrove, updateAlphaInTree)
...
getMeanAndSE <- function(treeeListList, idxCV, datX, response){

  error <- sapply(seq_along(treeeListList), function(i) sum(predict(object = treeeListList[[i]], newdata = datX[idxCV==i,,drop = FALSE]) != response[idxCV==i]))
...
getCutAlpha <- function(treeeList){
  ...
  return(max(alphaList,na.rm = TRUE))
}
